...
16. **The individual persists; its body changes.** A tuber, a leaf-form plant and a flowering individual can be the same persistent individual. Damage to a temporary manifestation ends with that manifestation; the persistent core carries harm across phases in **vitality**. **No harm is permanent while the individual lives** — classes differ in depth and recovery duration, never in recoverability; only death is terminal. **Bloom requires species-specific flowering maturity**, is repeatable, and is never unlocked with points. **Magic suspends biology; it does not consume it** — an inhabited Amorpho runs on magical state while its biology pauses, embodiment is open-ended, and **Astral Readiness** is restored by rooted life, never by leaving and re-entering. The **Anchor is a bridge, not a tracker**: Deep Dormancy silences the astral signal entirely. **Developmental Maturity** is one continuous, species-relative axis of persistent development — not XP, not age, not condition; it grows from biological surplus and regresses only on Core Impact, and **healing does not restore it** (AMO-D080–AMO-D089, [docs/19](docs/19_DEVELOPMENTAL_MATURITY_V0.md), [docs/20](docs/20_BIOLOGY_MAGIC_BOUNDARY_AND_ASTRAL_READINESS_V0.md)). The topology is seven states in three families, with Bloom a **sibling active state** rather than a linear stage — and **rooted, astrally exited and dormant are three different things**. Availability follows biological state; there is no roster timer. (AMO-D058, AMO-D059, AMO-D070–AMO-D078, [docs/15](docs/15_LIFE_CYCLE_ASTRAL_ANCHORS_AND_AVAILABILITY.md), [docs/17](docs/17_LIFE_CYCLE_STATE_MACHINE_V0.md), [docs/18](docs/18_PHASE_DAMAGE_CORE_RECOVERY_AND_BLOOM_MATURITY_V0.md))
