...
1. **Answer AMO-Q045 — is any harm permanent?** Every deferral now points here. It decides whether *vitality* must be stored at all (AMO-Q073, AMO-D056), whether death is a vitality threshold or a terminal state (AMO-D057), what a premature retreat can ultimately cost (AMO-Q087), and whether a regenerating fragment is the same individual (AMO-Q094). The condition model and the state machine were both built so either answer fits.
