...
1. **Give Remaining Productive Opportunity its first qualitative representation** — the one frontier every trace now stops at: what the concept actually consists of (external opportunity reachable in the World, biological opportunity remaining in the individual's lifecycle, and the manifestation's ability to use it), how it is expressed without becoming a visible countdown, and what changes it. Qualitative only, no rates or integration (AMO-Q116, AMO-D145, AMO-D146, [44_FULL_BIOLOGICAL_YEAR_PERSISTENT_STATE_INTEGRATION_TRACE_V1.md](44_FULL_BIOLOGICAL_YEAR_PERSISTENT_STATE_INTEGRATION_TRACE_V1.md)).
