...
1. **Answer AMO-Q108 — what makes harm cross into the persistent core.** It is now the last unfilled hinge in the biological model: maturity regression, vitality loss and premature retreat all name "Core Impact occurred" as their trigger without anything defining it (AMO-D079, AMO-D082). The subtle part is whether prolonged moderate impairment can accumulate into core impact with no single catastrophic event — if only discrete events can cross, a slow grinding season is free, which seems wrong.
