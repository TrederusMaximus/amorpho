...
1. **Specify active-phase productive return, qualitatively** — the smallest useful slice of AMO-Q116: what an active Leaf's biological work actually yields, which persistent things it can support (reserves, condition, developmental maturity), what bounds it (Functional Capacity and Remaining Productive Opportunity), and where it hands off to persistent loss (AMO-Q118) — with no formula, rate or value. Every trace from [23](23_ACUTE_EVENT_TO_LEAF_IMPAIRMENT_V0.md) to [38_END_TO_END_COMBAT_ENCOUNTER_AND_BIOLOGICAL_CONTINUITY_TRACE_V0.md](38_END_TO_END_COMBAT_ENCOUNTER_AND_BIOLOGICAL_CONTINUITY_TRACE_V0.md) now terminates in this question, and Tuber Core Viability depends on it (AMO-D131).
